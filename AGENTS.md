...
- **Categoria e localização de produto só vêm dos cadastros** (Estoque → Categorias / Localizações, migração 043). Nome único sem diferenciar caixa (índice em `lower(name)`; a Server Action também ignora acento), `active` tira o registro das novas escolhas sem mexer nos produtos que já o usam, e as FKs de `products` são `on delete restrict`. O gatilho `products_cadastro_ativo` recusa cadastro inativo em produto novo ou alterado. Nunca criar localização a partir de texto digitado. Regras em `src/lib/estoque/cadastros-servidor.ts`, uma tela para os dois (`CadastroDeApoio`).
...
- **O header é `sticky`, e quem rola é a janela.** Por isso o `main` usa `overflow-x-clip`: um `overflow-y-auto` ali cria um contêiner de rolagem que nunca rola, e o sticky gruda nele em vez de na tela. O header fica em `z-20`, abaixo do botão de recolher a sidebar (`z-30`), que se apoia sobre a barra.
- **Menu e pesquisa de funções têm uma lista só**: `src/components/layout/navegacao.ts`. Tela nova entra ali, com a mesma permissão que a página exige no servidor. `ATALHOS` guarda as telas fora do menu (criação, cadastros de apoio); `termos` são os sinônimos da busca. A pesquisa (`PesquisaDeFuncoes`, Ctrl+K ou "/") mostra só o que a sessão pode abrir.
